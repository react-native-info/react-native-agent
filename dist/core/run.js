...
import { defineInputGuardrail, defineOutputGuardrail, } from './guardrail';
...
import { getOrCreateTrace, resetCurrentSpan, setCurrentSpan, withNewSpanContext, withTrace, } from './tracing/context';
...
import { ServerConversationTracker, applyCallModelInputFilter, } from './runner/conversation';
import { createGuardrailTracker, runOutputGuardrails, } from './runner/guardrails';
import { adjustModelSettingsForNonGPT5RunnerModel, mergeModelSettings, maybeResetToolChoice, selectModel, } from './runner/modelSettings';
import { getResponseWithRetry, getStreamedResponseWithRetry, } from './runner/modelRetry';
import { processModelResponseAsync } from './runner/modelOutputs';
import { addStepToRunResult, streamStepItemsToRunResult, isAbortError, } from './runner/streaming';
import { createSessionPersistenceTracker, prepareInputItemsWithSession, saveStreamInputToSession, saveStreamResultToSession, saveToSession, } from './runner/sessionPersistence';
...
export class Runner extends RunHooks {
            ...
            try {
                while (true) {
                    ...
                    if (state._currentStep.type === 'next_step_run_again') {
                        ...
                        const processedResponse = await processModelResponseAsync(state._lastTurnResponse, state._currentAgent, preparedCall.tools, preparedCall.handoffs, state, [...preparedCall.turnInput, ...state._generatedItems]);
                        state._lastProcessedResponse = processedResponse;
                        await guardrailTracker.awaitCompletion();
                        const turnResult = await resolveTurnAfterModelResponse(state._currentAgent, state._originalInput, state._generatedItems, state._lastTurnResponse, state._lastProcessedResponse, this, state, toolErrorFormatter, options.errorHandlers);
                        applyTurnResult({
                            state,
                            turnResult,
        ...
        try {
            while (true) {
                ...
                if (result.state._currentStep.type === 'next_step_run_again') {
                    ...
                    const processedResponse = await processModelResponseAsync(result.state._lastTurnResponse, currentAgent, preparedCall.tools, preparedCall.handoffs, result.state, [...preparedCall.turnInput, ...result.state._generatedItems]);
                    ...
                    const turnResult = await resolveTurnAfterModelResponse(currentAgent, result.state._originalInput, result.state._generatedItems, result.state._lastTurnResponse, result.state._lastProcessedResponse, this, result.state, toolErrorFormatter, options.errorHandlers);
                    applyTurnResult({
                        state: result.state,
                        turnResult,
                        agent: currentAgent,
    ...
    async #prepareModelCall(state, options, artifacts, turnInput, serverConversationTracker, sessionInputUpdate) {
        ...
        let modelSettings = mergeModelSettings(this.config.modelSettings, state._currentAgent.modelSettings);
